...
class JasperSystem{
    ...
    private $memberMyinfoMode;
    private $memberWithdrawalMode;
    ...
    public function __construct($index, $arrPostVal, $connect){
        ...
        $this->memberMyinfoMode = null;
        $this->memberWithdrawalMode = null;
    ...
    public function __destruct(){
        ...
        unset($this->memberMyinfoMode);
        unset($this->memberWithdrawalMode);
    ...
    public function getMemberMyinfoMode(){
        return $this->memberMyinfoMode;
    }
    
    public function setMemberMyinfoMode($memberMyinfoMode){
        $this->memberMyinfoMode = $memberMyinfoMode;
    }
    
    public function getMemberWithdrawalMode(){
        return $this->memberWithdrawalMode;
    }
    
    public function setMemberWithdrawalMode($memberWithdrawal){
        $this->memberWithdrawalMode = $memberWithdrawal;
    }
    ...
    private function memberPage(){
        ...
        $memberMyinfoMode = $this->getMemberMyinfoMode();
        $memberWithdrawalMode = $this->getMemberWithdrawalMode();
        ...
        ### 내 정보(My info)
        else if (strpos($subPage, "myinfo") === 0 ){
            ...
            if ( $state == 1 ){
                ...
                // 내 정보 수정 시도 중일 떄
                if ( $memberMyinfoMode == 1 )
                {
                    $sessionElem = new MemberBaseModel();
                    
                    $sessionElem->setEmail($_SESSION["email"]);
                    $sessionElem->setLevel($_SESSION["level"]);
                    $member->myinfo_ok($sessionElem, $memberElem);
                }
                // 내 정보 페이지
                else{
                    
                    $memberElem->setEmail($_SESSION["email"]);
                    $memberElem->setLevel($_SESSION["level"]);
                    $member->myinfo($memberElem);
                }
        ...
        ### 약관(Agreement)
        else if (strpos($subPage, "agreement") === 0){
            
        }
        ### 개인정보보호정책(Privacy Agreement)
        else if (strpos($subPage, "privacy") === 0 ){
