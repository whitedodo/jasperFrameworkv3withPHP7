...
<table class="tg_general" style="width:100%;">
  ...
  <tr> 
    <td class="tg-nknw" style="background-color:#E2E2E2; width:10%;">
		...
		<td class="tg-nknw"><?php echo substr($element->getIP(), 0, 14); ?></td>
	</tr>
</table>
...
<!-- 파일 첨부 -->
<!-- File Attach -->
<h4>파일 첨부(File Attach)&nbsp;</h4>

<table class="tg_general" style="width:100%;">
<?php 
    if ( !empty($fileStack) ){
        foreach ( $fileStack as $val ){
?>

  	<tr>
		<td class="tg-nknw" style="background-color:#E2E2E2;width:7%;">
			<?php echo $val->getId(); ?>
		</td>
		<td class="tg-nknw" style="">
			<span style="font-weight:bold;">
				<?php echo "<a href=\"../downloader/" . $val->getId() . "\">"; ?>
				<?php echo $val->getFileName(); ?>
				<?php echo "</a>"; ?>
			</a>
			<?php echo "  (". $val->getFileSize() . ")"; ?><br>
			<?php echo "  등록일자(Regidate):". $val->getRegidate(); ?>
			</span>
		</td>
	</tr>
...
<h5><?php echo $passwdMsg; ?>&nbsp;</h5>
<?php 
    foreach ($commentStack as $val){
?>
<form name="comment_frm_<?php echo $val->getId(); ?>" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
	
	<table class="tg_general" style="width:100%; margin-top:10px;">
	<tr>
		<td class="tg-031e" style="width:8%;"><?php echo $val->getId(); ?></td>
		<td class="tg-031e" style="width:42%;">
		<textarea class="textarea_box" name="memo" rows="3" cols="20"><?php echo $val->getMemo(); ?></textarea></td>
		<td class="tg-031e" style="width:10%;"><?php echo $val->getAuthor(); ?></td>
		<td class="tg-031e" style="width:15%;"><?php echo $val->getRegidate(); ?>
			<br><?php echo $val->getIp(); ?></td>
		<td class="tg-031e"><input type="password" name="passwd" class="input_box">
			<span class="error">* </span></td>
		<td class="tg-031e" style="width:20%;">
			<input type="button" onclick="jasper_comment('<?php echo $boardName; ?>', '<?php echo $pageId; ?>', '<?php echo $val->getId(); ?>', 'm')" value="수정(Modify)" class="comment_handle_btn" style="color:#000;">
			<input type="button" onclick="jasper_comment('<?php echo $boardName; ?>', '<?php echo $pageId; ?>', '<?php echo $val->getId(); ?>', 'd')" value="삭제(Remove)" class="comment_handle_btn" style="color:#000;">
		</td>
	</tr>
	</table>
</form>
...
<input type="hidden" name="typeMode" value="w">
...
<script src="<?php echo $skin_dir; ?>js/jasperComment.js"></script>
