...
$BOARD_PROTECTED_MODE = $_POST['protectedMode'];

### 댓글 포스트
$COMMENT_POSTID = $_POST['commentId'];
...
$COMMENT_TYPE_MODE = $_POST['typeMode'];



### 게시글 글쓰기 모드 - 비밀번호
if ( strcmp( $BOARD_PASSWD1, $BOARD_PASSWD2) === 0 ){
    $BOARD_PASSWD = $BOARD_PASSWD1;
    
}else if ( strcmp( $BOARD_PASSWD1, $BOARD_PASSWD2 ) === -1 ){
    $BOARD_PASSWD = -1;
}else{
...
### 삭제 모드(게시물)
if ( $BOARD_REMOVE_MODE == 1)
{
    $BOARD_PASSWD = $_POST['passwd'];
}

### 글 보호 모드(게시물)
if ( $BOARD_PROTECTED_MODE == 1){
    $BOARD_PASSWD = $_POST['passwd'];
}

### 수정 모드 또는 삭제 모드(댓글)
if ($COMMENT_TYPE_MODE == 'm' || 
    $COMMENT_TYPE_MODE == 'd' )
{
...
#echo "야" . $BOARD_ELEMENT->getCategory() . "/";

### 댓글 모델
$COMMENT_ELEMENT = new CommentModel();

$COMMENT_ELEMENT->setId($COMMENT_POSTID);
